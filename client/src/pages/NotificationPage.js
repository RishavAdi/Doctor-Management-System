...
// Implement your logic for marking all notifications as read
  const handleMarkAllRead = async() => {
    try{
      dispatch(showLoading());
      const res=await axios.post('/api/v1/user/getallnotification',{
        userId:user._id,
      },{
        headers:{
...
// Implement your logic for deleting all read notifications
        ...
        <Tabs.TabPane tab="unRead" key={0}>
          ...
          </div>
          {
            user?.notification.map(notificationMgs => (
              <div className="card" style={{ cursor: "pointer" }} key={notificationMgs.doctorId}>
                <div
                  className="card-text"
